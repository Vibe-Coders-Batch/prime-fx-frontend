      ...
      <Card className="border-none shadow-lg bg-card/50 backdrop-blur-sm">
        <CardContent className="p-8">
            <div className="grid gap-6 md:grid-cols-3">
                <AnimatedRadioCard selected={formats.video ?? false} onClick={() => handleToggle('video')} title="Video Lessons" description="Pre-recorded video lectures for students to watch at their own pace." icon={<Video className="h-8 w-8"/>} className="h-full"/>

                <AnimatedRadioCard selected={formats.text ?? false} onClick={() => handleToggle('text')} title="Text / Articles" description="Written content, guides, and documentation for detailed learning." icon={<FileText className="h-8 w-8"/>} className="h-full"/>

                <AnimatedRadioCard selected={formats.quiz ?? false} onClick={() => handleToggle('quiz')} title="Quizzes" description="Assessments to test student knowledge and retention." icon={<CheckSquare className="h-8 w-8"/>} className="h-full"/>
            </div>
