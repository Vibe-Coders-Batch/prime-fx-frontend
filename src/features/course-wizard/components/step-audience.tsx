...
function DynamicList({ label, placeholder, items, onChange }: { label: string, placeholder: string, items: string[], onChange: (items: string[]) => void }) {
    const handleAdd = () => {
        onChange([...items, ""]);
    };
    ...
    return (
        <div className="space-y-4">
                ...
                {items.map((item, index) => (
                    <div key={index} className="flex gap-3 group">
                        <div className="flex-1 relative">
                            ...
                            />
                             {/* Optional check icon or status indicator could go here */}
                        </div>
                        <Button 
                            variant="ghost" 
                            size="icon" 
                            onClick={() => handleRemove(index)}
                            className="h-12 w-12 text-muted-foreground hover:text-destructive hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                            <X className="h-5 w-5" />
                        </Button>
                    </div>
            ...
            <Button 
                ...
                size="sm" 
                onClick={handleAdd} 
                className="mt-2 text-primary hover:text-primary border-dashed border-primary/20 hover:border-primary/50"
            >
    ...
    <div className="space-y-8">
      ...
      <Card className="border-none shadow-lg bg-card/50 backdrop-blur-sm">
        <CardContent className="p-8 space-y-12">
            <DynamicList 
                label="What will learners be able to do after completing this course?"
            ...
            <DynamicList 
                label="Are there any prerequisites?"
