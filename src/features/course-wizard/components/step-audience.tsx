...
function DynamicList({ label, placeholder, items, onChange }: {
    ...
    return (<div className="space-y-4">
            ...
            <div className="space-y-3">
                {items.map((item, index) => (<div key={index} className="flex gap-3 group">
                        <div className="flex-1 relative">
                            <Input value={item} onChange={(e) => handleChange(index, e.target.value)} onKeyDown={(e) => handleKeyDown(e, index)} placeholder={placeholder} className="h-12 pl-4 pr-10 text-base" autoFocus={index === items.length - 1 && index > 0}/>
                             
                        </div>
                        <Button variant="ghost" size="icon" onClick={() => handleRemove(index)} className="h-12 w-12 text-muted-foreground hover:text-destructive hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-opacity">
                            <X className="h-5 w-5"/>
                        </Button>
                    </div>))}
            </div>
            <Button type="button" variant="outline" size="sm" onClick={handleAdd} className="mt-2 text-primary hover:text-primary border-dashed border-primary/20 hover:border-primary/50">
...
export function StepAudience() {
    ...
    return (<div className="space-y-8">
       <div className="text-center space-y-2 mb-10">
         <h2 className="text-3xl font-bold tracking-tight">Target Audience</h2>
         <p className="text-muted-foreground text-lg">Define who your course is for and what they will achieve.</p>
       </div>
      ...
      <Card className="border-none shadow-lg bg-card/50 backdrop-blur-sm">
        <CardContent className="p-8 space-y-12">
            ...
            <div className="flex justify-between pt-8">
              ...
              <Button onClick={onNext} size="lg" className="px-8 rounded-full" disabled={updateCourse.isPending}>
