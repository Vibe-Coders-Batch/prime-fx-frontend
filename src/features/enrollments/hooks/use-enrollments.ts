...
export function useEnrollments({ enabled, refetchOnMount, filters }: UseEnrollmentsParams = {}) {
  const fetchEnrollments = async (): Promise<{ data: Enrollment[]; pagination: any }> => {
    ...
    const res = await axiosGet<{ data: Enrollment[]; pagination: any }>("enrollments", params);
    return res.data;
  };
  ...
  return useQuery({
    ...
    refetchOnMount: refetchOnMount ?? "always", // Always refetch on mount
...
export const useCreateEnrollment = () => {
  ...
  const fetchCreateEnrollmentMutationFunction = async (
...
export const useUpdateEnrollmentStatus = () => {
  ...
  const fetchUpdateEnrollmentStatusMutationFunction = async ({
    enrollmentId,
    status,
  }: {
    enrollmentId: string;
...
export function useCheckEnrollment({ enabled, courseId }: { enabled?: boolean; courseId: string }) {
  const fetchCheckEnrollment = async (): Promise<{ isEnrolled: boolean; enrollmentId?: string }> => {
    const res = await axiosGet<{ isEnrolled: boolean; enrollmentId?: string }>(`enrollments/check-access/${courseId}`);
    return res.data;
  };
