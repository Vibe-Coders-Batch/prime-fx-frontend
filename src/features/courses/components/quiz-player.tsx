        ...
        <div className="flex items-center justify-between">
           ...
           {isSubmitted && (
             <div className={cn("px-4 py-1.5 rounded-full text-sm font-bold border", 
                 score >= passingPercentage ? "bg-green-500/10 text-green-600 border-green-500/20" : "bg-red-500/10 text-red-600 border-red-500/20"
             )}>
             ...
             <div className="p-8 space-y-8">
                ...
                <div className="space-y-3">
                  {currentQuestion.answers.map((opt, optIdx) => {
                    ...
                    if (isSubmitted) {
                       if (opt.isCorrect) stateStyles = "border-green-500/50 bg-green-500/10 text-green-700 dark:text-green-300";
                       else if (isSelected && !opt.isCorrect) stateStyles = "border-red-500/50 bg-red-500/10 text-red-700 dark:text-red-300";
                    ...
                    return (
                      ...
                      >
                        ...
                        {isSubmitted && opt.isCorrect && (
                            <CheckCircle2 className="absolute right-4 h-5 w-5 text-green-600" />
                        )}
                ...
                {/* Explanation */}
                {isSubmitted && !currentQuestion.answers[answers[currentQuestion.quizQuestionId]]?.isCorrect && currentQuestion.explanation && (
                    <motion.div 
          ...
          <div className="p-6 bg-muted/20 border-t flex justify-between items-center">
             ...
             {isLastQuestion ? (
                 isSubmitted ? (
                     ...
                     <Button onClick={handleSubmit} className="px-8">Submit Quiz</Button>
                 )
             ) : (
                 <Button onClick={handleNext} disabled={answers[currentQuestion.quizQuestionId] === undefined}>
                    Next Question
                 </Button>
             )}
          </div>
