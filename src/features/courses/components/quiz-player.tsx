        ...
        <Card className="border-none shadow-lg bg-card/60 backdrop-blur-sm overflow-hidden">
          ...
          <div className="p-6 bg-muted/20 border-t flex justify-between items-center">
             ...
             {isLastQuestion ? (isSubmitted ? (score >= passingPercentage ? (<Button onClick={() => onComplete(score, true)} className="gap-2">
                         ...
                         </Button>)) : (<Button onClick={handleSubmit} className="px-8">Submit Quiz</Button>)) : (<Button onClick={handleNext} disabled={answers[currentQuestion.quizQuestionId] === undefined}>
                    Next Question
                 </Button>)}
          </div>
        </Card>
