...
export function CourseBuilder({ courseId }: CourseBuilderProps) {
  ...
  const [modulePriceType, setModulePriceType] = useState<
    "INCLUDED" | "INDIVIDUAL" | "BOTH"
  >("INCLUDED");

  const [isAddingLessonFor, setIsAddingLessonFor] = useState<string | null>(
    null
  );
  const [editingLessonId, setEditingLessonId] = useState<string | null>(null);
  const [previewLessonId, setPreviewLessonId] = useState<string | null>(null);
  const [lessonTitle, setLessonTitle] = useState("");
  const [lessonType, setLessonType] = useState<"VIDEO" | "TEXT" | "QUIZ">(
    "VIDEO"
  );
  ...
  const handleUpdateModule = async () => {
    if (!editingModuleId || !moduleTitle.trim()) return;
    try {
  ...
  const handleAddLesson = async () => {
    if (!isAddingLessonFor || !lessonTitle.trim()) return;
    try {
      const newLesson = await createLessonMutation.mutateAsync({
        ...
        type: lessonType,
        order:
          (modules?.find((m) => m.sectionId === isAddingLessonFor)?.lessons
            ?.length || 0) + 1,
      });
      ...
      // Auto-open editor to let user add content immediately
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  value={modulePriceType}
                  onValueChange={(val: "INCLUDED" | "INDIVIDUAL" | "BOTH") =>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                onClick={
                  editingModuleId ? handleUpdateModule : handleCreateModule
                }
                disabled={
                  createModuleMutation.isPending ||
                  updateModuleMutation.isPending
      ...
      <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-4">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => confirmDeleteModule(module.sectionId)}
              ...
              <div className="px-4 pb-4 space-y-2">
                {module.lessons && module.lessons.length > 0 ? (
                  module.lessons.map((lesson: Lesson) => (
                    <div
                      key={lesson.lessonId}
                      className="flex items-center justify-between p-2 rounded-md bg-muted/50 ml-6 pl-2"
                    >
                      ...
                      <div className="flex items-center gap-1">
                        {lesson.type === "VIDEO" && (
                          <Button
                            ...
                            size="icon"
                            className="h-6 w-6"
                            onClick={() => setPreviewLessonId(lesson.lessonId)}
                            title="Preview video"
                          >
                        ...
                        <Button
                          ...
                          size="icon"
                          className="h-6 w-6"
                          onClick={() => confirmDeleteLesson(lesson.lessonId)}
                ...
                <div className="ml-8 mt-2">
                  <Dialog
                    open={isAddingLessonFor === module.sectionId}
                    onOpenChange={(open) => !open && setIsAddingLessonFor(null)}
                  >
                    ...
                    <DialogContent>
                      ...
                      <DialogFooter>
                        <Button
                          variant="outline"
                          onClick={() => setIsAddingLessonFor(null)}
        ...
        {(!modules || modules.length === 0) && !isAddingModule && (
