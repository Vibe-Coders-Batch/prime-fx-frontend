...
export function LessonEditorDialog({ open, onOpenChange, lessonId, courseId }: LessonEditorDialogProps) {
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col p-0 gap-0">
        <DialogHeader className="px-8 py-6 border-b bg-muted/40 backdrop-blur-md">
        ...
        {isLoading ? (<div className="flex-1 flex justify-center items-center">
            ...
            </div>) : (<div className="flex-1 overflow-y-auto p-8 space-y-8">
                
                <div className="space-y-4">
                    <Label className="text-base font-medium">Lesson Title</Label>
                    <Input value={title} onChange={(e) => setTitle(e.target.value)} className="h-14 text-lg font-medium transition-all" placeholder="e.g. Introduction to Forex"/>
                </div>
                ...
                {lesson?.type === "VIDEO" && (<div className="space-y-6 pt-2">
                        ...
                        <VideoUpload courseId={courseId} lessonId={lessonId} currentUrl={videoUrl} onUploadComplete={(url) => setVideoUrl(url)} onDurationChange={(d) => setDuration(d)}/>
                        ...
                        {lessonId && (lesson?.status === 'READY' || lesson?.status === 'PROCESSING') && (<div className="space-y-2 pt-4 border-t">
                ...
                {lesson?.type === "TEXT" && (<div className="space-y-4 pt-2">
                        ...
                        <Textarea className="font-mono text-sm min-h-[400px] leading-relaxed p-6" value={textContent} onChange={(e) => setTextContent(e.target.value)} placeholder="# Write your lesson here..."/>
                    </div>)}

                {lesson?.type === "QUIZ" && (<div className="space-y-6 pt-2">
        ...
        <div className="p-6 border-t bg-muted/40 flex justify-end gap-3">
