...
export const couponsApi = {
  ...
  activate: async (couponId: string): Promise<{ couponId: string; isActive: boolean }> => {
    const { data } = await apiClient.patch<{ couponId: string; isActive: boolean }>(
      `/admin/coupons/${couponId}/activate`
    );
    return data;
  },

  deactivate: async (couponId: string): Promise<{ couponId: string; isActive: boolean }> => {
    const { data } = await apiClient.patch<{ couponId: string; isActive: boolean }>(
  ...
  delete: async (couponId: string): Promise<{ couponId: string; isDeleted: boolean }> => {
    const { data } = await apiClient.delete<{ couponId: string; isDeleted: boolean }>(
  ...
  validateBulk: async (payload: {
    ...
    const { data } = await apiClient.post<BulkCouponValidationResult>(
      '/coupons/validate-bulk',
      payload
    );
    return data;
  },
