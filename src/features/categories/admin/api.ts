...
export const adminCategoriesApi = {
  ...
  activate: async (categoryId: string): Promise<{ categoryId: string; isActive: boolean }> => {
    const { data } = await apiClient.patch<{ categoryId: string; isActive: boolean }>(
      `/admin/categories/${categoryId}/activate`
    );
    return data;
  },

  deactivate: async (categoryId: string): Promise<{ categoryId: string; isActive: boolean }> => {
    const { data } = await apiClient.patch<{ categoryId: string; isActive: boolean }>(
  ...
  delete: async (categoryId: string): Promise<{ categoryId: string; isDeleted: boolean }> => {
    const { data } = await apiClient.delete<{ categoryId: string; isDeleted: boolean }>(
