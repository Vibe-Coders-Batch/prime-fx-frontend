...
export default function AdminDashboardPage() {
    ...
    return (<PageLayout header="Admin Dashboard" description="Monitor platform metrics, analytics, and manage all aspects of the system.">
      <div className="space-y-6">
        ...
        {stats && !isLoading ? (<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <StatCard title="Total Users" description="All platform users" value={(stats as any)?.totalUsers || 0} icon={Users} gradient="from-blue-500 to-cyan-500"/>
            <StatCard title="Total Courses" description="Published courses" value={(stats as any)?.totalCourses || 0} icon={BookOpen} gradient="from-purple-500 to-pink-500"/>
            <StatCard title="Total Enrollments" description="Course enrollments" value={(stats as any)?.totalEnrollments || 0} icon={UserCheck} gradient="from-green-500 to-emerald-500"/>
            <StatCard title="Revenue" description="Total platform revenue" value={`$${((revenueStats as any)?.total || 0).toFixed(2)}`} icon={DollarSign} gradient="from-amber-500 to-orange-500"/>
          </div>) : isLoading ? (<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <StatsCardSkeleton />
            <StatsCardSkeleton />
            <StatsCardSkeleton />
            <StatsCardSkeleton />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="grid gap-6 md:grid-cols-2">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="grid gap-6 md:grid-cols-2">
