...
export default function LearnerDashboardPage() {
    ...
    const continueWatchingCourses = useMemo(() => {
        if (!enrollmentsData?.data || !allCourses?.data)
            return [];
        const publishedCoursesMap = new Map(allCourses.data.map((course) => [course.courseId, course]));
    ...
    const featuredCourses = useMemo(() => {
        ...
        if (courses.length <= 10) {
            const different = courses.filter((course) => !newReleasesIds.has(course.courseId));
            if (different.length > 0) {
                return different.slice(0, 10);
            }
            return courses.slice().reverse().slice(0, 10);
        }
        const differentCourses = courses.filter((course) => !newReleasesIds.has(course.courseId));
    ...
    const categoryCarousels = useMemo(() => {
        if (!categories || categories.length === 0 || !allCourses?.data)
            return [];
        return categories.slice(0, 3).map((category) => {
            const categoryCourses = allCourses.data.filter((c) => c.categoryId === category.categoryId);
            return {
                category,
    ...
    if (coursesLoading || categoriesLoading) {
        return (<div className="space-y-6 px-4 sm:px-6 md:px-8 lg:px-12 py-8">
    ...
    return (<PageTransition>
      <div className="space-y-0 relative overflow-hidden">
        ...
        <motion.div className="absolute inset-0 -z-10 opacity-30" animate={{
            background: [
        ...
        {heroCourse && (<motion.div className="-mx-4 sm:-mx-6 md:-mx-8 lg:-mx-12" initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: "easeOut" }}>
            <HeroBanner course={heroCourse}/>
          </motion.div>)}
        <div className="space-y-3 px-4 sm:px-6 md:px-8 lg:px-12 relative">
          {continueWatchingCourses.length > 0 && (<motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.4, duration: 0.6, ease: [0.4, 0, 0.2, 1] }}>
              <ContinueWatchingCarousel courses={continueWatchingCourses}/>
            </motion.div>)}

          {featuredCourses && featuredCourses.length > 0 && (<motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.5, duration: 0.6, ease: [0.4, 0, 0.2, 1] }}>
              <CourseCarousel title={<motion.span className="flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }}>
                    <motion.div animate={{ rotate: [0, 360] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }}>
                      <Sparkles className="h-5 w-5 text-primary"/>
                    </motion.div>
                    Featured Courses
                  </motion.span>} courses={featuredCourses}/>
            </motion.div>)}

          {newReleasesCourses && newReleasesCourses.length > 0 && (<motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.6, duration: 0.6, ease: [0.4, 0, 0.2, 1] }}>
              <CourseCarousel title={<motion.span className="flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 }}>
                    <motion.div animate={{ y: [0, -5, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}>
                      <TrendingUp className="h-5 w-5 text-primary"/>
                    </motion.div>
                    New Releases
                  </motion.span>} courses={newReleasesCourses}/>
            </motion.div>)}

          {categoryCarousels.map(({ category, courses }, index) => {
            if (courses.length === 0)
                return null;
            return (<motion.div key={category.categoryId} initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.7 + index * 0.1, duration: 0.6, ease: [0.4, 0, 0.2, 1] }}>
                <CourseCarousel title={`Popular in ${category.name}`} courses={courses}/>
              </motion.div>);
        })}

          {enrollmentsData?.data && enrollmentsData.data.length === 0 && !enrollmentsLoading && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
