...
export default function CourseProgressPage({ params, }: {
    params: Promise<{
        courseId: string;
    }>;
}) {
    ...
    return (<PageLayout header={`Course Progress: ${course.title}`} subtitle="Learning" description="Track your progress through this course.">
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            const completedInSection = sectionLessons.filter((lesson) => progress?.lessonProgress?.find((lp) => lp.lessonId === lesson.lessonId && lp.completed)).length;
            const sectionProgress = sectionLessons.length > 0
                ? (completedInSection / sectionLessons.length) * 100
                : 0;
            return (<div key={section.sectionId} className="space-y-3">
                    ...
                    const lessonProgress = progress?.lessonProgress?.find((lp) => lp.lessonId === lesson.lessonId);
                    return (<div key={lesson.lessonId} className="flex items-center gap-3 p-2 rounded hover:bg-accent transition-colors">
                            {lessonProgress?.completed ? (<CheckCircle className="h-5 w-5 text-green-600 shrink-0"/>) : (<Clock className="h-5 w-5 text-muted-foreground shrink-0"/>)}
                            <span className="flex-1 text-sm">
                              {lessonIdx + 1}. {lesson.title}
                            </span>
                            {lesson.duration && (<span className="text-xs text-muted-foreground">
