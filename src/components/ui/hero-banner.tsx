...
export function HeroBanner({ course, className }: HeroBannerProps) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative w-full h-[40vh] sm:h-[80vh] md:h-[85vh] lg:h-[90vh] overflow-hidden group",
        className
      )}
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col justify-end pb-8 sm:pb-16 md:pb-20 lg:pb-24 px-4 sm:px-8 md:px-12 lg:px-20 xl:px-32">
        <motion.div
          ...
          className="max-w-3xl space-y-3 sm:space-y-5 md:space-y-6"
        >
          <motion.div
          ...
          >
            {course.category && (
              <span className="px-3 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-sm font-bold bg-primary text-primary-foreground rounded-full shadow-lg">
                <Sparkles className="inline h-2.5 w-2.5 sm:h-3 sm:w-3 mr-1 sm:mr-1.5" />
                {course.category.name}
              </span>
            )}
            {totalDuration > 0 && (
              <span className="px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-sm font-semibold bg-white/10 text-white rounded-full backdrop-blur-md flex items-center gap-1 sm:gap-1.5">
          ...
          <motion.h1
            ...
            className="text-xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-white leading-[1.1] drop-shadow-2xl tracking-tight"
          >
            {course.title}
          </motion.h1>
          ...
          {course.description && (
            <motion.p
              ...
              className="text-xs sm:text-base md:text-lg text-white/95 line-clamp-2 sm:line-clamp-3 max-w-2xl drop-shadow-xl font-medium leading-relaxed"
            >
              {course.description}
            </motion.p>
          )}
          ...
          <motion.div
            ...
            className="flex flex-row items-center gap-3 sm:gap-5 pt-2 sm:pt-4"
          >
            <Link
              href={`/learner/courses/${course.courseId}/watch`}
              className="group/button flex-1 sm:flex-none"
            >
              <Button
                size="lg"
                className="w-full sm:w-auto bg-white text-black hover:bg-white/90 text-sm sm:text-lg px-4 sm:px-10 py-5 sm:py-8 rounded-xl font-bold gap-2 sm:gap-3 shadow-2xl hover:shadow-white/20 transition-all duration-300 hover:scale-105"
              >
            ...
            >
              <Button
                variant="outline"
                size="lg"
                className="w-full sm:w-auto bg-white/10 text-white border-2 border-white/40 hover:bg-white/20 text-sm sm:text-lg px-4 sm:px-10 py-5 sm:py-8 rounded-xl font-bold gap-2 sm:gap-3 backdrop-blur-md hover:border-white/60 transition-all duration-300 hover:scale-105"
              >
