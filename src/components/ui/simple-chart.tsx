...
export function SimpleBarChart({ title, data, maxValue, height = 200, noCard = false }: SimpleBarChartProps) {
  ...
  const chartContent = (
    <div 
      className="space-y-3 sm:space-y-2 md:space-y-3 w-full min-h-full" 
    >
      {data.map((item, index) => {
        const percentage = max > 0 ? (item.value / max) * 100 : 0;
        return (
          <div key={item.label} className="space-y-2 sm:space-y-1 w-full">
...
export function SimpleLineChart({ title, data, height = 200, noCard = false }: SimpleLineChartProps) {
  ...
  const chartContent = (
    <div style={{ height: `${height}px` }} className="relative flex flex-col w-full max-w-full">
            ...
            return (
              ...
              />
                 ...
                 <div className="h-6 flex justify-between items-start text-[10px] sm:text-xs text-muted-foreground pt-1 px-0.5 sm:px-1 gap-0.5 sm:gap-1 overflow-hidden">
                   {data.map((item, index) => (
                     <span key={index} className="truncate max-w-[45px] sm:max-w-[60px] flex-shrink-0" title={item.label}>
                       {item.label}
                     </span>
                   ))}
                 </div>
