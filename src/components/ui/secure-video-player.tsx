...
export function SecureVideoPlayer({ lessonId, videoKey, className, autoPlay = false, showStatus = false }: SecureVideoPlayerProps) {
    ...
    if (isLoading) {
        return (<div className={cn("flex items-center justify-center bg-black/90 aspect-video rounded-md", className)}>
        <Loader2 className="h-8 w-8 text-white animate-spin"/>
        <span className="ml-2 text-white text-sm">Loading secure content...</span>
      </div>);
    }
    if (error || !signedUrl) {
        return (<div className={cn("flex flex-col items-center justify-center bg-gray-900 aspect-video rounded-md text-white p-4", className)}>
        <Lock className="h-8 w-8 mb-2 text-red-400"/>
        <p className="text-sm mb-3">{error || "Video not found"}</p>
        
        {lessonId && (<div className="flex gap-2">
        ...
        {debugInfo && (<div className="mt-4 p-3 bg-gray-800 rounded-md text-xs text-left w-full max-w-md overflow-auto max-h-48">
            ...
            {debugInfo.encodingJob ? (<>
                ...
                {debugInfo.encodingJob.errorMessage && (<p><span className="text-red-400">Error:</span> {debugInfo.encodingJob.errorMessage}</p>)}
    ...
    return (<div className={cn("relative", className)}>
      <EnhancedVideoPlayer src={signedUrl} className="w-full" autoPlay={autoPlay} onError={(err) => {
            setError(err);
            fetchDebugInfo();
        }}/>
    </div>);
