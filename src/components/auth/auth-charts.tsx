    ...
    <div className="relative w-72 h-40">
      ...
      <div className="absolute bottom-0 left-8 flex items-end gap-3 h-full pb-px pl-px z-10">
         <motion.div 
            ...
            className="w-4 bg-blue-600/80 rounded-t-sm backdrop-blur-sm"
         />
         <motion.div 
            initial={{ height: 0 }}
            animate={{ height: "65%" }}
            transition={{ duration: 0.8, delay: 0.4 }}
            className="w-4 bg-yellow-500/90 rounded-t-sm backdrop-blur-sm shadow-[0_0_15px_rgba(234,179,8,0.3)]"
         />
      </div>
      ...
      <svg className="absolute inset-0 w-full h-full overflow-visible z-20">
        <motion.path
          d="M 20 130 C 50 130, 60 100, 90 110 C 120 120, 140 80, 170 90 C 200 100, 220 40, 260 50"
          fill="none"
          stroke="#EAB308"
          strokeWidth="3"
          strokeLinecap="round"
          ...
          style={{ filter: "drop-shadow(0px 0px 8px rgba(234,179,8,0.5))" }}
        />
...
export const AnimatedBarChart = () => {
    return (
      <div className="relative w-64 h-48 flex items-end justify-center gap-6 pb-8">
         {/* Background glow */}
         ...
         <motion.div 
            ...
            className="w-10 bg-gradient-to-t from-blue-700 to-blue-500 rounded-md shadow-lg border border-blue-400/20"
         />
         <motion.div 
             initial={{ height: 0, opacity: 0 }}
             animate={{ height: "70%", opacity: 1 }}
             transition={{ duration: 0.8, ease: "backOut", delay: 0.4 }}
             className="w-10 bg-gradient-to-t from-yellow-600 to-yellow-400 rounded-md shadow-xl shadow-yellow-500/10 border border-yellow-400/20 relative group"
         >
         ...
         <svg className="absolute inset-0 w-full h-full pointer-events-none z-10" viewBox="0 0 256 192">
            <motion.path 
                d="M40 150 L100 110 L160 50 L200 40" 
                fill="none" 
                stroke="#E3B558" 
                strokeWidth="2" 
                strokeLinecap="round"
