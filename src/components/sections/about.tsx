...
export function About() {
    return (<section className="py-10 sm:py-12 md:py-16 bg-background relative overflow-hidden" id="about" aria-labelledby="about-heading">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <header className="text-center mb-10 sm:mb-12 md:mb-16 max-w-4xl mx-auto">
          <motion.h2 id="about-heading" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-2xl sm:text-3xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/70 mb-4 sm:mb-6">
            About Prime Learning
          </motion.h2>

          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="space-y-4 sm:space-y-6 text-muted-foreground text-sm sm:text-base md:text-lg leading-relaxed">
            <p>
        ...
        <div className="grid md:grid-cols-2 gap-4 sm:gap-6 md:gap-8 mb-12 sm:mb-16 md:mb-24" role="group" aria-label="Mission and Vision">
          ...
          <motion.article initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="bg-card border border-border/50 rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-8 hover:border-blue-500/50 transition-colors duration-300">
        ...
        <div aria-labelledby="values-heading">
          <motion.h3 id="values-heading" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-xl sm:text-2xl md:text-3xl font-bold text-center mb-8 sm:mb-10 md:mb-12">
            Our Core Values
          </motion.h3>

          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 list-none p-0" role="list" aria-label="Core values">
            {VALUES.map((value, i) => (<motion.li key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
