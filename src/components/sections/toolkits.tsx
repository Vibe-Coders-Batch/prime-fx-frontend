...
function ToolkitItem({ title, headline, description, index, imageSrc, imageAlt, }: {
    ...
    return (<article ref={ref} className={`flex flex-col md:flex-row items-center gap-8 sm:gap-10 md:gap-16 lg:gap-24 py-8 sm:py-10 md:py-12 ${isEven ? "" : "md:flex-row-reverse"}`}>
      <div className="flex-1 space-y-4 sm:space-y-6 text-center md:text-left">
        <header>
          <p className="flex items-center justify-center md:justify-start gap-3 text-primary-gold mb-2">
...
export function Toolkits() {
    return (<section id="toolkits" className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 bg-background" aria-labelledby="toolkits-heading">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-12 sm:space-y-16 md:space-y-24" role="list" aria-label="Professional trading tools">
          <ToolkitItem index={0} title="Risk Calculator" headline="Never Over-Leverage Again." description="Our dynamic position size calculator adjusts to live market volatility and your account balance instantly." imageSrc="/illustrations/risk_calculator.svg" imageAlt="Risk Calculator tool interface showing position sizing calculations"/>
          <ToolkitItem index={1} title="Trade Journal" headline="The Professional's Edge." description="Track your psychological state, entry/exit logic, and performance metrics. The only way to improve is to measure." imageSrc="/illustrations/analytics_setup.svg" imageAlt="Trade Journal interface showing performance analytics and trade tracking"/>
        </div>
      </div>
    </section>);
}
