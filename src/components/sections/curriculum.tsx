...
function TiltCard({ children, className = "", }: {
    ...
    return (<motion.div ref={ref} onMouseMove={onMouseMove} onMouseLeave={onMouseLeave} style={{ transform, transformStyle: "preserve-3d" }} className={`group relative rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 p-5 sm:p-6 md:p-8 backdrop-blur-sm transition-colors hover:border-primary-gold/50 ${className}`}>
...
export function Curriculum() {
    return (<section id="curriculum" className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 bg-background relative overflow-hidden" aria-labelledby="curriculum-heading">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] sm:w-[800px] md:w-[1000px] h-[300px] sm:h-[400px] md:h-[500px] bg-primary-gold/5 rounded-full blur-[80px] sm:blur-[100px] md:blur-[120px] pointer-events-none" aria-hidden="true"/>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 list-none p-0" role="list" aria-label="Course tiers">
                ...
                <article className="flex flex-col h-full">
                  ...
                  <ul className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 flex-grow list-none p-0" role="list" aria-label={`${tier.name} tier features`}>
                    {tier.features.map((feature, j) => (<li key={j} className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-muted-foreground">
                        <Check className="w-4 h-4 sm:w-5 sm:h-5 text-primary-gold shrink-0" aria-hidden="true"/>
                        <span>{feature}</span>
                      </li>))}
                  </ul>
