...
export const useUpdateProgress = () => {
  ...
  const fetchUpdateProgressMutationFunction = async ({
    courseId,
    dto,
  }: {
    courseId: string;
...
export function useGetUploadKey() {
  return useMutation({
    mutationFn: ({ type, fileName, courseId }: { type: 'course' | 'profile' | 'lesson'; fileName?: string; courseId?: string }) =>
...
export function useUploadFile() {
  return useMutation({
    mutationFn: ({ file, key }: { file: File; key: string }) => storageApi.upload(file, key),
  });
}
